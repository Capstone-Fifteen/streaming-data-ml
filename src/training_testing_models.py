...
def create_callback(
    model: Model, path_chpt, logger=None, patience=250, metric="accuracy", verbose=5, epochs=None):
    """callback settinngs
    Args:
        model (Model)
    ...
    callbacks.append(
        EarlyStopping(monitor="loss", min_delta=0, patience=patience, verbose=1, mode="min", restore_best_weights=True))
    
    callbacks.append(
        ModelCheckpoint(filepath=f"{path_chpt}", save_best_only=True))
...
def initialize_scoring(option="general_method"):
    global modes
    
    if option=="general_method": 
        modes = ["train", "valid", "test", "macro averaged"]
    elif option=="no_test": 
        modes = ["train", "valid", "macro averaged"]
    elif option=="no_cv": 
        modes = ["train", "test", "macro averaged"]
    scores = dict()
...
def rename_file(old_file_name): 
    file_list = glob.glob(old_file_name)
    if len(file_list) > 0:
        cur_time = datetime.now().strftime("%d-%b-%Y-%H-%M-%S_")
        head, tail = os.path.split(old_file_name)
        os.rename(old_file_name, os.path.join(head, "renameAt_"+cur_time+tail))


def export_summary(df_1, df_2, prefix, model_names, INFO, export_mode="test"):
    model_name_agg = "_".join(model_names)
    csv_file_name = f"{LOGS_DIR}/{INFO}/AllModels/{prefix}_freq_{sampling_freq}_ws_{window_size}_split_{n_splits}_{model_name_agg}[{export_mode}].xlsx"

    rename_file(csv_file_name)
    writer_summary = pd.ExcelWriter(csv_file_name, engine='xlsxwriter')
    
    df_1_name = f"{LOGS_DIR}/{INFO}/AllModels/{prefix}_freq_{sampling_freq}_ws_{window_size}_split_{n_splits}_clf_summary[{export_mode}].pickle"
    df_2_name = f"{LOGS_DIR}/{INFO}/AllModels/{prefix}_freq_{sampling_freq}_ws_{window_size}_split_{n_splits}_cv_df_meta[{export_mode}].pickle"
    
    rename_file(df_1_name)
    rename_file(df_2_name)
    ...
    try: 
        ...
        utils.pickle_save(df_1_name, df_1)
        utils.pickle_save(df_2_name, df_2)
    except: 
        display(df_1)
        display(df_2)
...
def train_and_predict(LOG_DIR, key, X_train, X_valid, y_train, y_valid, logger=None, X_test=None,
                      clf_name="", fold_id="no_cv", params=None, pdf=None):
    """Train CNN
    Args:
    ...
    if is_NN_model(clf_name):
        ...
        model.save(f"{TEST_DIR}/BestModel/trained_model_dataset_{key}_model_{clf_name}_fold({fold_id})")
        model.save(f"{LOG_DIR}/trained_model/trained_model_dataset_{key}_model_{clf_name}_fold({fold_id})")
...
def train_cross_validate_no_test(
    ...
    scores = initialize_scoring(option="no_test")
    ...
    for key in sorted(new_dic.keys()):
            ...
            if fold_id >= skip_fold: 
                ...
                if is_NN_model(clf_name):
                    pred_tr, pred_val, pred_test, model = train_and_predict(
                        LOG_DIR, key, X_tr, X_val, y_tr, y_val, 
                        logger=logger, X_test=None, clf_name=clf_name, fold_id=fold_id, params=params_model, pdf=pdf)
                else:
                    pred_tr, pred_val, pred_test, model = train_and_predict(
                        LOG_DIR, key, X_tr, X_val, y_tr, y_val, 
                        logger=logger, X_test=None, clf_name=clf_name, fold_id=fold_id)
        ...
        for mode in ["train", "valid"]:
            ...
            cv_df = pd.DataFrame.from_dict(cv, orient='index', columns=[f"Fold {i+1}" for i in range(n_splits-skip_fold)]).T
            ...
            for class_id in range(num_classes):
                mean_class_f1 = np.mean([class_f1_mat[i][class_id] for i in range(n_splits-skip_fold)])
...
def train_and_predict_noCV(
    ...
    classifiers_summary_datasets = pd.DataFrame()
    cv_df_meta_datasets = pd.DataFrame()
    scores = initialize_scoring(option="no_cv")
    ...
    for key in sorted(new_dic.keys()):
        pdf = PdfPages(f"{LOG_DIR}/images/dataset_{key}_model_{clf_name}_cross_validation.pdf")
        # writer = pd.ExcelWriter(f'{LOG_DIR}/cross_validation/dataset_{key}_model_{clf_name}_cv.xlsx', engine='xlsxwriter')
        ...
        print(f"_____________________{Dataset_name} Training______________________")
        logger.debug(f"_____________________{Dataset_name} Training ______________________")
        ...
        logger.debug(f"{clf_name} started training....")
        ...
        test_preds = np.zeros((1, X_test.shape[0], num_classes)) if is_NN_model(clf_name) else np.zeros(
                (1, X_test.shape[0]))
        
        y_train = keras.utils.to_categorical(y_train, num_classes) if is_NN_model(clf_name) else y_train
        y_test = keras.utils.to_categorical(y_test, num_classes) if is_NN_model(clf_name) else y_test
        ...
        if is_NN_model(clf_name):
            pred_tr, pred_val, pred_test, model = train_and_predict(
                LOG_DIR, key, X_train, X_test, y_train, y_test,
                logger=logger, clf_name=clf_name, X_test=None, params=params_model, pdf=pdf)
        else:
            pred_tr, pred_val, pred_test, model = train_and_predict(
                LOG_DIR, key, X_train, X_test, y_train, y_test,
                logger=logger, clf_name=clf_name, X_test=None)
        
        for pred, X, y, mode in zip([pred_tr, pred_val], [X_train, X_test],
        ...
        # print("________Section 1________")
        
        # for mode in ["train", "test"]:
        #     print("________Section 1A________")
        #     logger.debug(f"________{mode.upper()} confusion table________")
        #     cv = dict((k, scores[key][mode][k]) for k in ["accuracy", "precision", "recall", "f1_score", "log_loss"])
        #     cv_df = pd.DataFrame.from_dict(cv, orient='index', columns=["main_fold"]).T
        #     cols_1 = cv_df.columns
        #     cv_df_meta = pd.concat([cv_df_meta, cv_df], axis=1)
            
        #     print("________Section 1B________")
        #     cv_calc = [mode]+[np.array(cv[i]).mean() for i in cv.keys()]
        #     cv_calc_s = pd.Series(cv_calc, index=["Mode"]+["Mean "+i for i in cv.keys()])
        #     classifiers_summary = classifiers_summary.append(cv_calc_s, ignore_index=True)
            
        #     print("________Section 1C________")
        #     columns = list(Activities_index_name.values())
        #     index = list(Activities_index_name.values())
        ...
        # # Output Final Scores Averaged over Folds
        # print("________Section 2________")

        # test_preds[0]=pred_val
        # test_pred = np.mean(test_preds, axis=0).argmax(axis=1) if is_NN_model(clf_name) else get_freq_along_axis(test_preds, axis=1)
        # y_test_max = y_test.argmax(axis=1) if is_NN_model(clf_name) else y_test
        ...
        # print("________Section 3________")

        # cv_df_meta.columns = pd.MultiIndex.from_product([["train", "test"], cols_1])
        ...
        # print("________Section 4________")
        ...
        #         labels=list(Activities_index_name.values()),
        #         path=f"{LOG_DIR}/images/confusion_matrix_dataset_{key}_model_{clf_name}.pdf", pdf_write=pdf,
        #         clf_name=clf_name,option="no_cv")
...
def main_training_testing(model_names, train_test_files_dic, Activities_index_name, INFO, intended_dataset_number,
    ...
    for clf_name in model_names:
        ...
        if option=="general_method": 
            ...
            train_and_predict_noCV(
                clf_name, LOG_DIR, train_test_files_dic, Activities_index_name,
                intended_dataset_number, logger, dataset_type=dataset_type)
            export_mode="no_cv"
