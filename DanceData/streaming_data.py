# -*- coding: utf-8 -*-
"""Step_1_2_Milestone2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1lP6hXkj3Bw4fJhw_SIE6_iNvT35qSK8w
"""
...
separator = "========================================================"
separator_small = "_______________"
...
LOG_FILE_DIR = f"{TEST_DIR}/Predictions/predictions_log"
...
LOG_FILE_DIR_summary = f"{TEST_DIR}/Predictions/predictions_csv"
...
logger.debug(f"{separator}Starting to predict streaming data{separator}")
...
def model_list_predict(model_dict, dataset_type, X_train, writer, start_index=-1, end_index=-1, act_ID=-1, mode="streaming"): 
    meta_string=""
    dict_list = []
    ...
    for model_name, model in model_dict[dataset_type].items(): 
        ...
        if np.isnan(pred.any()): 
            ...
            pred_dict = pd.DataFrame(pred, columns=[f"CLASS {i}[{Activities_index_name[i]}]" for i in range(10)]).to_dict('records')[0]
            ...
            pred_model_name = f"\n\n{separator_small}{model_name} is used:{separator_small}\n"
            pred_final_result = f"{separator_small}{separator_small}{max_pred[0]}[{Activities_index_name[max_pred[0]]}] with prob={highest_prob}\n"
    ...
    pred_model_name_summary = f"\n\n{separator_small}SUMMARY_TABLE_ACTIVITY_PROPORTION{separator_small}\n"
    pred_final_result_summary = f"{separator_small}{separator_small}Most Likely Activity: {most_likely_act}\n"
    ...
    if mode=="streaming": 
        ...
        logger_summary.debug(f"{start_index}-{end_index}: Most Likely Activity: {most_likely_act}][prob={summary_dict[most_likely_act]}]")
...
def predict_streaming(): 
    try: 
        ...
        Dataset_type_III, file_name = load.create_raw_dic(
            ...
            scaler_name=f"{TEST_DIR}/Scaler/scaler_Chi_wk8(3)[standard][AccX_AccY_AccZ_GyroYaw_GyroPitch_GyroRoll]", 
            mode="streaming"
        )
        ...
        model_list_predict(model_dict, 3, X_train_III, writer, start_index=index_list[0], end_index=index_list[1])
...
def predict_file(file_name = ""): 
    ...
    logger.debug(f"{separator}Starting to STREAMING{separator}")
    start_time=timer()
    for idx in tqdm(range(X_train_III.shape[0])): 
        ...
        X_train_III_elem = X_train_III_elem.reshape(1, X_train_III_elem.shape[0], X_train_III_elem.shape[1], X_train_III_elem.shape[2])
        model_list_predict(model_dict, 3, X_train_III_elem, writer, start_index, end_index, act_ID, mode="streaming")
    
    end_time=timer()
    logger.debug(f'Duration in seconds : {end_time-start_time}')
    logger.debug(f"{separator}Ending process STREAMING{separator}")
