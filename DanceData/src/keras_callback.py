...
logger = getLogger(f"{LOGS_DIR}/{prefix}ws_{window_size}_split_{n_splits}_models.log")
...
class PeriodicLogger(Callback):
    ...
    def __init__(
        self, metric: str = "accuracy", verbose: int = 0, epochs: Optional[int] = None
    ) -> None:
        self.metric = metric
    ...
    def on_epoch_end(self, epoch: int, logs: Optional[Dict[str, float]] = None) -> None:
        ...
        if epoch % self.verbose == 0:
            ...
            logger.debug(msg)
def create_callback(
    model: Model, path_chpt: str, patience: int = 80, metric: str = "accuracy", verbose: int = 0, epochs: Optional[int] = None
) -> List[Any]:
    """callback settinngs
    Args:
        model (Model)
